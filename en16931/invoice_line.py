...
class InvoiceLine:
    ...
    def __init__(
        ...
        line_extension_amount: float | None = None,
    ...
    def discount(self, value) -> None:
        ...
        discount = parse_money(value, self._currency)

        # pas de réduction négative
        if discount.amount < 0:
            raise ValueError("La réduction ne peut pas être négative.")

        # pas plus que le montant de la ligne
        if self._price is not None and self._quantity is not None:
            line_amount = self._price * self._quantity
            if discount > line_amount:
                raise ValueError("La réduction ne peut pas être > au montant de la ligne.")

        self._discount = discount

        # Recalcul du total
        if self._price is not None and self._quantity is not None:
            self._line_extension_amount = self._compute_line_extension_amount()
    ...
    def line_extension_amount(self):
        ...
        return self._compute_line_extension_amount()
    ...
    def _compute_line_extension_amount(self):
        """
        Calcul centralisé pour prendre en compte un discount au niveau des lignes
        """
        if self._price is None or self._quantity is None:
            return None

        amount = self._price * self._quantity

        if self._discount:
            amount -= self._discount
